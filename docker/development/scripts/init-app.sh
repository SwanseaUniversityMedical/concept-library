...
if [ ! -z $DEBUG ] && [ $DEBUG = 0 ]; then
  ...
  python /var/www/CodeListLibrary_project/manage.py collectstatic --noinput --clear --ignore=*.scss
  python /var/www/CodeListLibrary_project/manage.py compress
  python /var/www/CodeListLibrary_project/manage.py collectstatic --noinput --ignore=*.scss
