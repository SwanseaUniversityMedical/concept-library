...
class ConceptDetail(LoginRequiredMixin, HasAccessToViewConceptCheckMixin, DetailView):
    ''' 
        Display the concept details.
        The concept/detail.html template requires information about which of
        the components are visible to the user. A list of ids is provided in
        the context data.
    ''' 
    model = Concept
    template_name = 'clinicalcode/concept/detail.html'    
    ...
    def get_context_data(self, **kwargs):
        ...
        context['user_can_edit'] = (not self.get_object().is_deleted) and \
            allowed_to_edit(self.request.user, Concept, self.get_object().id)
        ...
        context['user_can_export'] = (allowed_to_view_children(self.request.user, Concept, self.get_object().id)
                                      ...
                                      and 
                                      not self.get_object().is_deleted 
                                      )
        context['allowed_to_create'] = not settings.CLL_READ_ONLY

        ids_query = PublishedConcept.objects.filter(concept_id=self.get_object().id).values_list('concept_history_id')
        historical_ids = [item for t in ids_query for item in t] 
        context['published_historical_ids'] = historical_ids
...
class ConceptHistoryView(LoginRequiredMixin, HasAccessToViewConceptCheckMixin, DetailView):
    ''' display a history view of a concept '''
    model = Concept
    template_name = 'clinicalcode/concept/history/detail.html'
    ...
    def get_object(self, queryset=None):
        ...
        history_concept = obj.history\
            .get(history_id=self.kwargs['concept_history_id'])\

        self.transaction_time = history_concept.history_date

        return history_concept.instance

    def get_context_data(self, **kwargs):
        context = DetailView.get_context_data(self, **kwargs)

        history_components = Component.objects.filter(id=self.kwargs['pk'])

        context['components'] = history_components.history.as_of(self.transaction_time)
...
@login_required
def concept_history_detail(request, pk, concept_history_id):
    ''' 
    ...
    if concept['owner_id'] is not None:
        ...
        for user in users:
            if (user.id == int(concept['owner_id'])):
                concept['owner'] =  user
                break
    if concept['group_id'] is not None: 
        groups = Group.objects.all()
        for group in groups:
            if (group.id == int(concept['group_id'])):
                concept['group'] = group
                break
    ...
    return render(request, 'clinicalcode/concept/history/detail.html',
                  {'concept': concept, 
                   'components': components, 
                   'tags': tags,
...
def concept_list(request):
    ...
    # Distinct moved to get_visible_concepts.
    # concepts = Concept.objects.distinct()
    '''
     only(
        "id", "name", "coding_system__name", "author", "modified",
        "modified_by__username", "created", "created_by__username",
        "is_deleted", "deleted", "concept.concepttagmap.tag.description"
        ).
    '''
    ...
    # Run through the concepts and add a 'can edit this concept' field.
        ...
        if form.is_valid():
            ...
            try:
                with transaction.atomic():
					...
					#??
                    data['allowed_to_publish'] = db_utils.allowed_to_publish(request.user, Concept, pk)
...
class ConceptPublish(LoginRequiredMixin, HasAccessToViewConceptCheckMixin, TemplateResponseMixin, View):
    ...
    def get(self, request, pk, concept_history_id):
        concept = Concept.objects.get(pk=pk)
        history_concept = concept.history.get(history_id=concept_history_id)
        is_published = db_utils.checkIfPublished(pk, concept_history_id)
        allow_to_publish = allowed_to_publish(self.request.user, Concept, pk, historical_id=concept_history_id)
        
        return self.render_to_response({'pk': pk, 'name': concept.name, 'concept_history_id': concept_history_id, 'is_published': is_published, 'allowed_to_publish': allow_to_publish})
