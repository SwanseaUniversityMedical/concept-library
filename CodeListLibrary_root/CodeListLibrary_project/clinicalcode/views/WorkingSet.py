...
def workingset_create(request):
    ...
    if request.method == 'GET':
        ...
        new_workingset.concept_version = db_utils.getWSConceptsHistoryIDs(request.POST.get('concepts_json'))
    ...
    def post(self, request, pk):
        ...
        if not is_valid:    # pop the form with data to correct and submit                
            ...
            with transaction.atomic():
                ...
                # save the tag ids
                old_tag_list = list(WorkingSetTagMap.objects.filter(workingset=workingset).values_list('tag', flat=True))
                ...
                for tag_id_to_add in tag_ids_to_add:
                    WorkingSetTagMap.objects.get_or_create(workingset=workingset, tag=Tag.objects.get(id=tag_id_to_add), created_by=self.request.user)
...
class WorkingSetDelete(LoginRequiredMixin, HasAccessToEditWorkingsetCheckMixin, TemplateResponseMixin, View):
    ...
    def get(self, request, pk):
        workingset = WorkingSet.objects.get(pk=pk)
        return render(request, self.template_name, {'pk': pk, 'name': workingset.name})
...
class WorkingSetDetail(LoginRequiredMixin, HasAccessToViewWorkingsetCheckMixin, DetailView):
...
def workingset_history_revert(request, pk, workingset_history_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            data['message'] = render_to_string(
                                'clinicalcode/workingset/history/revert.html',
                                {}, request)
            return JsonResponse(data)
...
#     rows = db_utils.getGroupOfConceptsByWorkingsetId(pk)
# 
#     my_params = {
#         'workingset_id': pk,
...
#     response['Content-Disposition'] = ('attachment; filename="workingset_%(workingset_id)s_concepts_%(creation_date)s.csv"' % my_params)
# 
...
#         rows_no=0
#         codes = db_utils.getGroupOfCodesByConceptId(concept_id)
...
def workingset_list(request):
    '''
        display a list of workingsets. This view can be searched and contains paging
    '''
    ...
    concept_brand = request.GET.get('concept_brand', request.session.get('concept_brand_ws', request.CURRENT_BRAND))
    ...
    request.session['concept_brand_ws'] = concept_brand
    ...
    # Run through the workingsets and add a 'can edit this workingset' field.
