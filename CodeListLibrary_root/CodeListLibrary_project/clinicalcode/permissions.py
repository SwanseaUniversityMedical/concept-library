...
'''
    ...
    ---------------------------------------------------------------------------
'''
def allowed_to_view_children(user, set_class, set_id, returnErrors = False, WS_concepts_json = ""):
    '''
    ...
    from db_utils import getConceptTreeByConceptId, getGroupOfConceptsByWorkingsetId, getConceptsFromJSON
    errors = dict()
...
def get_visible_components(user, codeListID):
    ...
    if codeListID is not None:
        ...
        codelist = CodeList.objects.get(id=codeListID)
        components = Component.objects.filter(id=codelist.component_id)
        concepts = get_visible_concepts(user).filter(id=components[0].concept_id)
    else:
        # Start with them all!
        components = Component.objects.distinct()
        concepts = get_visible_concepts(user)
...
def get_visible_workingsets(user):
    ...
    if user.is_superuser: return workingsets
