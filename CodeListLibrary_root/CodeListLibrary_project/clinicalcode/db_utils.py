...
def forkHistoryConcept(user, concept_history_id):
    ...
    if concept_obj:
        ...
        components = getHistoryComponents(concept['id'], concept_history_date)
...
def getConceptTreeByConceptId(concept_id):
    ...
    with connection.cursor() as cursor:
        #strSQL = "SELECT * FROM get_concept_tree_by_concept_id(%s);" % concept_id
...
def getParentConceptTreeByConceptId(concept_id):
    ...
    '''
    with connection.cursor() as cursor:
#         strSQL = "SELECT * FROM get_parent_concept_tree_by_concept_id(%s);" % concept_id
#         cursor.execute(strSQL)
...
def getHistoryConceptTagMaps(concept_id, concept_history_date):
    ''' Get historic concept tag maps that are attached to a concept that were effective from a point in time '''
...
def getHistoryComponents(concept_id, concept_history_date):
    '''
...
def chk_deleted_children(user, set_class, set_id, returnErrors = True, WS_concepts_json = ""):
    '''
        check if there are any deleted children of a concept or a working
    ...
    # Now check all the unique Concepts for deletion.
    AllnotDeleted = True
...
def chk_children_permission_and_deletion(user, set_class, set_id, WS_concepts_json = ""):
    '''
        check if there are any deleted/ or not permitted children
            of a concept or a working.
    ...
    is_permitted_to_all , error_perms = allowed_to_view_children(user, set_class, set_id, returnErrors=True, WS_concepts_json=WS_concepts_json)
    children_not_deleted , error_del = chk_deleted_children(user, set_class, set_id, returnErrors=True, WS_concepts_json=WS_concepts_json)
