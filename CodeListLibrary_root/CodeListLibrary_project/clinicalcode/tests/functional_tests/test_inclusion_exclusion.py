...
class InclusionExclusionTest(TestCase):
    ...
    def test_published(self):
        if settings.ENABLE_PUBLISH:
            from clinicalcode.publicsites.views import published_concept_codes_to_csv
            url = ('%s%s%s' % ('/publicsites/concepts/',
                               self.published_root.concept_history_id, '/export/codes'))
    
            request = self.factory.get(url)
            request.user = self.owner_user
    
            response = published_concept_codes_to_csv(request, self.published_root.concept_history_id)
