...
def getGroupOfConceptsByWorkingsetId_historical(workingset_id, workingset_history_id=None):
    ...
    concept_informations = json.loads(
        WorkingSet.history.get(id=workingset_id, history_id=workingset_history_id).concept_informations
        ...
        for component in components:
            ...
            if component['component_type'] == 1:  # concept
                ...
                if check_published_child_concept:
                    component['is_published'] = checkIfPublished(Concept, component['concept_ref_id'],
...
def build_sql_parameters(rules):
    ...
    for cond in rules['rules']:
        ...
        if 'condition' not in cond:
            if cond['operator'] == 'is_null' or cond['operator'] == 'is_not_null' or cond['operator'] == 'is_empty' or \
...
def create_codelist_codes(component_type, database_connection_name, table_name, code_field, desc_field, search_text
    ...
    insert_historical_sql = get_codes_insert_query(code_field, code_list_id, desc_field, table_name, where_sql,
...
def update_codelist_codes(component_type, database_connection_name, table_name, code_field, desc_field, search_text
    ...
    update_sql8_dic = get_codes_insert_query(code_field, code_list_id, desc_field, table_name, where_sql_dic,
                                             current_date, user_id)
    paramList.extend(update_sql8_dic['paramList'])

    update_sql = update_sql1 + update_sql2 + update_sql3 + update_sql4 + update_sql5 + update_sql6 + update_sql7 + \
                 update_sql8_dic['strSQL']
...
def update_expression_codes(component_type, database_connection_name, table_name, code_field, desc_field
                            , search_text, search_params, column_search, regex_type, regex_code, code_list_id, user_id,
                            filter
                            , case_sensitive_search):
    paramList = []
    ...
    sql_1 = get_codes_insert_query(code_field, code_list_id, desc_field, table_name, where_sql_dic, current_date,
                                   user_id)
...
def chk_deleted_children(request, set_class, set_id, returnErrors=True
    ...
    if (set_class == WorkingSet):
        ...
        if (not permitted):
            ...
            # the concepts here to form a list of concept_ref_ids.
        if WS_concepts_json.strip() != "":
            concepts = [x['concept_id'] for x in
                        json.loads(WS_concepts_json)]  # getConceptsFromJSON(concepts_json=WS_concepts_json)
        else:
...
def chk_children_permission_and_deletion(request, set_class, set_id, WS_concepts_json="", set_history_id=None,
...
def get_all_levels_codes(df1, concept_id, level=1, includeExclude=None):
    ...
    # -------------------
    child_concepts = df[(df['component_type'] == 1) & (df['level_depth'] == level) & (
            df['concept_id'] == concept_id)]  # & (df['code'] != '')
...
def getGroupOfCodesByConceptId_HISTORICAL(concept_id, concept_history_id):
    ...
    '''
    df = pd.DataFrame(
        columns=['logical_type', 'code', 'description', 'component_name', 'component_id', 'component_type'])
...
def isValidWorkingSet(request, working_set):
    ...
    if not working_set.publication_link.isspace() and len(
            working_set.publication_link) > 0 and not working_set.publication_link is None:
...
def get_history_child_concept_components(concept_id, concept_history_id=None):
    ...
    if concept_history_id is None:
        # live version
        conceptComp = Component.objects.filter(concept_id=concept_id, component_type=1).values('concept_ref_id',
...
def get_list_of_visible_entity_ids(data, return_id_or_history_id="both"):
    ...
    - data: list of dic is the output of ()
...
def getGroupOfConceptsByPhenotypeId_historical(phenotype_id, phenotype_history_id=None):
    ...
    concept_id_version = []
    concept_informations = json.loads(
        Phenotype.history.get(id=phenotype_id, history_id=phenotype_history_id).concept_informations)
...
def get_CodingSystems_from_Phenotype_concept_informations(concept_informations):
    ...
    CodingSystem_ids = Concept.history.filter(id__in=concept_id_list,
...
def isValidPhenotype(request, phenotype):
    ...
    if not phenotype.publication_link and len(
            phenotype.publication_link) > 0 and not phenotype.publication_link is None:
        validate = URLValidator()
        try:
...
# ---------------------------------------------------------------------------
def getHistory_ConceptCodeAttribute(concept_id, concept_history_date, code_attribute_header,
                                    expand_attrs_into_cols=False):
...
def getConceptCodes_withAttributes_HISTORICAL(concept_id, concept_history_date, allCodes, code_attribute_header):
    ...
    code_attributes = []
    if code_attribute_header:
        code_attributes = getHistory_ConceptCodeAttribute(concept_id, concept_history_date, code_attribute_header,
