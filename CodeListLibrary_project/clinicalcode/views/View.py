...
def index_HDRUK(request):
    ...
    if Statistics.objects.all().filter(org__iexact='HDRUK',
        ...
        stat = Statistics.objects.get(org__iexact='HDRUK',
...
#             HDRUK_stat = stat_obj[0]
    ...
    return render(
        ...
        'clinicalcode/brand/HDRUK/index_HDRUK.html',
        {
            ...
            HDRUK_stat['published_concept_count'],
            'published_phenotype_count':
            HDRUK_stat['published_phenotype_count'],
            'published_clinical_codes':
            HDRUK_stat['published_clinical_codes'],
            'datasources_component_count':
            HDRUK_stat['datasources_component_count'],
            'clinical_terminologies':
            HDRUK_stat['clinical_terminologies']
        })
...
# HDR-UK about pages
    if request.CURRENT_BRAND == "HDRUK":
        if pg_name.lower() == "hdruk_about_the_project".lower():
            return render(
                request,
                'clinicalcode/brand/HDRUK/about/about-the-project.html', {})
            ...
            return render(
                request,
                'clinicalcode/brand/HDRUK/about/covid-19-response.html', {})

        elif pg_name.lower() == "hdruk_about_publications".lower():
            return render(request,
                          'clinicalcode/brand/HDRUK/about/publications.html',
                          {})
...
#             return render(request, 'cl-docs/terms-conditions.html', {})
            ...
            return render(request,
                          'clinicalcode/brand/HDRUK/collections/breathe.html',
                          {})

        elif pg_name.lower() == "bhf_data_science_centre".lower():
            return render(
                request,
                'clinicalcode/brand/HDRUK/collections/bhf-data-science-centre.html',
                {})
    ...
    if unique_url is not None:
        phenotype = list(
            Phenotype.objects.filter(
                ...
                | Q(source_reference__iendswith=("/" +
                                                 unique_url))).values_list(
                                                     'id', flat=True))
        if phenotype:
            versions = Phenotype.objects.get(
                pk=phenotype[0]).history.all().order_by('-history_id')
            for v in versions:
                is_this_version_published = False
                is_this_version_published = db_utils.checkIfPublished(
                    Phenotype, v.id, v.history_id)
...
def build_permitted_components_list(request,
    ...
    for component in components:
        ...
        component.concept_history_id = Concept.objects.get(
            id=concept_id).history.latest().pk
        component.component_history_id = Component.objects.get(
            id=component.id).history.latest().pk
        ...
        if component.component_type == 1:
            ...
            if Concept.objects.get(
                    pk=component.concept_ref_id).is_deleted == True:
            ...
            if not allowed_to_view(
                    request,
                    Concept,
                    component.concept_ref.id,
                    set_history_id=component.concept_ref_history_id):
                component_error_msg_view[component.id] += [
                    "no view permission"
                ]
            ...
            if component.concept_ref_history_id != Concept.objects.get(
                    id=component.concept_ref_id).history.latest().pk:
                component_concpet_version_msg[component.id] += [
    ...
    data = {
        ...
        'latest_history_id':
        Concept.objects.get(id=concept_id).history.latest().pk
    }
    return data
...
def customRoot(request):
    ...
    urls_available = {
        'export_concept_codes':
        reverse('api:api_export_concept_codes', kwargs={'pk': 0}),
        'export_concept_codes_byVersionID':
        reverse('api:api_export_concept_codes_byVersionID',
                kwargs={
                    'pk': 0,
                    'concept_history_id': 123
                }),
        'api_export_published_concept_codes':
        reverse('api:api_export_published_concept_codes',
                kwargs={
                    'pk': 0,
                    'concept_history_id': 123
                }),
        'concepts':
        reverse('api:concepts', kwargs={}),
        'api_concept_detail':
        reverse('api:api_concept_detail', kwargs={'pk': 0}),
        'api_concept_detail_version':
        reverse('api:api_concept_detail_version',
                kwargs={
                    'pk': 0,
                    'concept_history_id': 123
                }),
        'api_published_concepts':
        reverse('api:api_published_concepts', kwargs={}),
        'api_concept_detail_public':
        reverse('api:api_concept_detail_public', kwargs={'pk': 0}),
        'api_concept_detail_version_public':
        reverse('api:api_concept_detail_version_public',
                kwargs={
                    'pk': 0,
                    'concept_history_id': 123
                }),
        'get_concept_versions':
        reverse('api:get_concept_versions', kwargs={'pk': 0}),
        'get_concept_versions_public':
        reverse('api:get_concept_versions_public', kwargs={'pk': 0}),
        'concept_by_id':
        reverse('api:concept_by_id', kwargs={'pk': 0}),
        'api_published_concept_by_id':
        reverse('api:api_published_concept_by_id', kwargs={'pk': 0}),
        'export_workingset_codes':
        reverse('api:api_export_workingset_codes', kwargs={'pk': 0}),
        'export_workingset_codes_byVersionID':
        reverse('api:api_export_workingset_codes_byVersionID',
                kwargs={
                    'pk': 0,
                    'workingset_history_id': 123
                }),
        'workingsets':
        reverse('api:workingsets', kwargs={}),
        'api_workingset_detail':
        reverse('api:api_workingset_detail', kwargs={'pk': 0}),
        'api_workingset_detail_version':
        reverse('api:api_workingset_detail_version',
                kwargs={
                    'pk': 0,
                    'workingset_history_id': 123
                }),
        'get_workingset_versions':
        reverse('api:get_workingset_versions', kwargs={'pk': 0}),
        'workingset_by_id':
        reverse('api:workingset_by_id', kwargs={'pk': 0}),
        ...
        'api_export_phenotype_codes_byVersionID':
        reverse('api:api_export_phenotype_codes_byVersionID',
                kwargs={
                    'pk': 0,
                    'phenotype_history_id': 123
                }),
        'phenotypes':
        reverse('api:phenotypes', kwargs={}),
        'api_phenotype_detail':
        reverse('api:api_phenotype_detail', kwargs={'pk': 0}),
        'api_phenotype_detail_version':
        reverse('api:api_phenotype_detail_version',
                kwargs={
                    'pk': 0,
                    'phenotype_history_id': 123
                }),
        'api_published_phenotypes':
        reverse('api:api_published_phenotypes', kwargs={}),
        # not needed to be public
        #'api_phenotype_detail_public': reverse('api:api_phenotype_detail_public', kwargs={'pk': 0}),
        'api_phenotype_detail_version_public':
        reverse('api:api_phenotype_detail_version_public',
                kwargs={
                    'pk': 0,
                    'phenotype_history_id': 123
                }),
        'api_export_published_phenotype_codes':
        reverse('api:api_export_published_phenotype_codes',
                kwargs={
                    'pk': 0,
                    'phenotype_history_id': 123
                }),
        ...
        'phenotype_by_id':
        reverse('api:phenotype_by_id', kwargs={'pk': 0}),
        'api_published_phenotype_by_id':
        reverse('api:api_published_phenotype_by_id', kwargs={'pk': 0}),
        'api_phenotype_detail_public':
        reverse('api:api_phenotype_detail_public', kwargs={'pk': 0}),
        'api_phenotype_detail_version':
        reverse('api:api_phenotype_detail_version',
                kwargs={
                    'pk': 0,
