...
class Phenotype(TimeStampedModel):
    ...
    phenotype_uuid = models.CharField(
        max_length=250)  # Unique ID for the phenotype on HDR UK platform
    type = models.CharField(max_length=250)
    validation_performed = models.NullBooleanField(
    )  # Was there any clinical validation of this phenotype?  1=yes 0=no
    validation = models.CharField(max_length=3000)
    valid_event_data_range = models.CharField(max_length=250,
                                              null=True,
                                              blank=True)
    ...
    phenoflowid = models.CharField(max_length=100, null=True,
                                   blank=True)  # ID to link to PhenoFlow
    ...
    tags = ArrayField(models.IntegerField(), blank=True,
                      null=True)  #default=list
    clinical_terminologies = ArrayField(models.IntegerField(),
                                        blank=True,
                                        null=True)  # coding systems
    publications = ArrayField(models.CharField(max_length=500),
                              blank=True,
                              null=True)
