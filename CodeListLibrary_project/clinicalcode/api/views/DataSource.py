    ...
    if request.method == 'POST':
        ...
        if not is_valid:
            ...
            if not DataSource.objects.filter(name__iexact=new_datasource.name.strip()).exists():
                ...
                existed_id = DataSource.objects.get(
                    name__iexact=new_datasource.name.strip()).id
                data = {
                    'message':
...
def get_LIVE_phenotypes_associated_with_data_source(
    ...
    phenotypes = Phenotype.objects.filter(
        id__in=phenotype_ids).order_by('name')
    ...
    for p in phenotypes:
        phenotype_latest_history_id = Phenotype.objects.get(
            pk=p.id).history.latest().history_id

        if show_published_data_only:
            is_published = checkIfPublished(Phenotype, p.id,
...
def get_HISTORICAl_phenotypes_associated_with_data_source(
    ...
    for p_id in associated_phenotype_ids:
        ver_to_return = []
        # get versions obj
        associated_phenotype_versions = Phenotype.objects.get(
            pk=p_id).history.all().order_by('-history_id')
        max_version_id = associated_phenotype_versions.aggregate(
            Max('history_id'))['history_id__max']
        include_this_phenotype = False
        for ver in associated_phenotype_versions:
            if show_published_data_only:
                is_published = checkIfPublished(Phenotype, p_id,
                                                ver.history_id)
                if not is_published:
                    continue

            data_sources_history = getHistoryDataSource_Phenotype(
                p_id, ver.history_date)
            if data_sources_history:
                phenotype_ds_list = [
                    i['datasource_id'] for i in data_sources_history
        ...
        if include_this_phenotype:
            rows_to_return.append(
                ordr(list(zip(ph_titles, ['PH' + str(p_id), ver_to_return]))))
