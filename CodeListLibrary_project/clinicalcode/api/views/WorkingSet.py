                    ...
                    ordr(
                        list(
                            zip(final_titles, [
                                ...
                                str(concept_id), concept_version[concept_id],
                                concept_name, current_ws_version.friendly_id,
                                current_ws_version.history_id,
                                current_ws_version.name
                            ] + data))))
...
def api_workingset_create(request):
    ...
    if request.method == 'POST':
        ...
        #-----------------------------------------------------------
            ...
            data = {
                'message': 'Workingset created successfully',
                'id': created_WS.pk
            }
...
def workingsets(request, pk=None):
    ...
    rows_to_return = []
    titles = [
        'workingset_id', 'workingset_name', 'latest_version_id', 'author',
...
def workingset_detail(request,
    ...
    if tags_comp:
        ...
        tags = list(
            Tag.objects.filter(pk__in=tag_list).values('description', 'id',
                                                       'tag_type',
                                                       'collection_brand'))
    ...
    if (ws['is_deleted'] == True
            or WorkingSet.objects.get(pk=pk).is_deleted == True):
        ret += [True]
    else:
        ret += [None]
...
def get_workingset_codes(request, pk, workingset_history_id):
    ...
    titles = (
        ['code', 'description', 'concept_id'] + ['concept_version_id'] +
        ['concept_name']
        #+ ['working_set_id' , 'working_set_version_id' , 'working_set_name']
        + title_row)
